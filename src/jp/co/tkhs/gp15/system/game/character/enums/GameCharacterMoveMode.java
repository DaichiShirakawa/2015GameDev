...
public enum GameCharacterMoveMode {
    ...
    UNLIMITED {
        @Override
        public float move(int displayBorder, int characterSize, float p, float vp) {
            return p + vp;
        }
    },
    /**
    ...
    REFRECTION_CENTER {
        @Override
        public float move(int displayBorder, int characterSize, float p, final float vp) {
            p += vp;
            if (p < 0) {
                p = -p;
            }
    ...
    /**
     * テクスチャが画面外に出た時点でキャラクターをdisposeする。 ※disposeはキャラクター実装任せ。
     */
     ...
     */
    public abstract float move(int displayBorder, int characterSize, float p, float vp);
}
