 ...
 * ゲームオブジェクトの実装クラス
 *
 * ひとつのオブジェクトは、自分を含めた複数の子オブジェクトを所有できる。 updateやrenderは自身およびすべての子に対して行われる。
...
public abstract class GameObjectImpl implements GameObject {
    ...
    /**
     * 自身のメイン処理を記述する。 falseを返せば入力処理と子のアップデートは行われない。 ポーズ処理など、一時的に止めたい場合にfalseを返す。
     */
    abstract protected boolean updateProcess();

    /**
     * update中に呼ばれる入力処理。 必要に応じてオーバーライドする。 falseを返せば子のアップデートは行われない。
     */
    ...
    /**
     * 自身の破棄処理を記述する。 テクスチャ破棄の追加が必要ならオーバーライドする。
     */
    ...
    @Override
    public <G extends GameObject> boolean add(G go) {
        if (go == null) {
    ...
    @Override
    public void booking(int bookingFrame) {
