...
public abstract class TSWeaponBase extends ShootingRotateCharacter {
    ...
    public TSWeaponBase(TokiShootingScene scene, ShootingRotateCharacter owner, LR equipLR) {
        super(scene, 0f);
        this.owner = owner;
        this.remainBullet = 0;
    ...
    public void reloadUpdate() {
        ...
        if (0 < remainReloadDelayFrame) {
            lastReloadFrame = FPSManager.getTotalFrame();
    ...
    private boolean reloadContinue() {
        return lastReloadFrame == FPSManager.getTotalFrame() - 1;
    }

    abstract protected float getDistanceFromOwner();

    public int getMaxDelayFrame() {
        if (lastReloadFrame == FPSManager.getTotalFrame()) {
